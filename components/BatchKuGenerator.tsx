...
const BatchKuGenerator: React.FC = () => {
  const { t } = useLanguage();
  const { apiKey, isApiKeySet } = useApiKey();
  
  const [batchKuInput, setBatchKuInput] = useState<string>(t('batchKuPlaceholder'));
  ...
  const handleParseAndLoad = useCallback(() => {
    setError(null);
    const input = batchKuInput.trim();
    if (!input) {
      setError(t('batchKuInputEmpty'));
      return;
    }

    // Attempt to parse as JSON first
    try {
      const cleanedJsonString = input
        .split('\n')
      ...
      if (parsed && Array.isArray(parsed.scenes)) {
        const extractedPrompts = parsed.scenes
          .map((scene: any) => scene.prompt)
          .filter((p: any): p is string => typeof p === 'string' && p.trim() !== '');

        if (extractedPrompts.length > 0) {
          setPrompts(extractedPrompts);
        } else {
          setError(t('noValidPromptsInBatchKu'));
        }
      } else {
        throw new Error("Not a valid story JSON format.");
      }
    } catch (e) {
      // Fallback to plain text
      const plainPrompts = input
        .split('\n')
        .map(p => p.trim())
        .filter(p => p && !p.startsWith('//'));

      if (plainPrompts.length > 0) {
        setPrompts(plainPrompts);
      } else {
        setError(t('noValidPromptsInBatchKu'));
      }
    }
  ...
  const handleDownload = (url: string, jobIndex: number) => {
    ...
    link.href = url;
    link.download = `ct-batch-ku-job-${jobIndex + 1}.mp4`;
        ...
        <form onSubmit={handleSubmit} className="space-y-6 flex flex-col">
            <fieldset disabled={isLoading} className="space-y-6 flex-grow">
              ...
              <div className="space-y-2">
                 <textarea value={batchKuInput} onChange={e => setBatchKuInput(e.target.value)} rows={10} className="w-full bg-slate-800/70 border border-gray-700 text-white rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-green-500 font-mono text-xs" />
                 <Button type="button" variant="secondary" onClick={handleParseAndLoad} className="w-full justify-center">{t('parseAndLoad')}</Button>
                 {prompts.length > 0 && <p className="text-center text-sm text-green-500">{t('promptsLoaded').replace('{count}', String(prompts.length))}</p>}
              </div>
...
export default BatchKuGenerator;
