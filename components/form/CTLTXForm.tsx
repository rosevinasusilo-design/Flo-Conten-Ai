...
const GENRES = ["Sci-Fi", "Fantasy", "Horror", "Comedy", "Drama", "Action", "Thriller", "Documentary", "Animation", "Pixar 3D", "Romance", "Custom..."];
...
const CTLTXForm: React.FC<CTLTXFormProps> = ({
    ...
    const isVeo3Model = useMemo(() => ltxProject.videoModel.startsWith('veo-3.0'), [ltxProject.videoModel]);
    ...
    useEffect(() => {
        ...
        if (isVeo3Model && currentAspectRatio !== '16:9') {
            updates.aspectRatio = '16:9';
        }
        
        if (ltxProject.videoModel.includes('veo-3.0-fast') && currentResolution !== '720p') {
    ...
    }, [ltxProject.videoModel, ltxProject.aspectRatio, ltxProject.resolution, updateProject, isVeo3Model]);

    const aspectRatioOptions = useMemo(() => {
        if (isVeo3Model) return ['16:9'];
        return ['16:9', '9:16'];
    }, [isVeo3Model]);
    ...
    const handleGenerateScript = async () => {
        ...
        try {
            ...
            const [outlineResponse]: [GenerateContentResponse, any] = await executeApiCallWithKeyRotation(async (ai) => {
                addLog('[CT-LTX] Langkah 1/2: AI Director sedang membuat kerangka alur cerita...', 'status');
                const charactersPrompt = ltxProject.characters.length > 0
                    ? `\n\n**Karakter yang Telah Ditentukan:**\n${ltxProject.characters.map(c => `- ${c.name}: ${c.description}. Pakaian: ${c.clothing}.`).join('\n')}`
                    : '';
                const estimatedScenes = Math.ceil(parseFloat(ltxProject.duration) * (ltxProject.durationUnit === 'minutes' ? 2.5 : 0.1));
                const directorOutlinePrompt = `Anda adalah seorang sutradara dan penulis naskah film AI. Berdasarkan spesifikasi, buatlah kerangka alur cerita (plot outline) yang dibagi menjadi ${estimatedScenes} adegan utama. Untuk setiap adegan, berikan ringkasan singkat tentang apa yang terjadi. ${charactersPrompt}
    
                Spesifikasi:
                - Judul: ${ltxProject.title}, Genre: ${ltxProject.genre}, Gaya Visual: ${ltxProject.visualStyle}
    
                FORMAT OUTPUT KRITIS: Kembalikan hasilnya sebagai objek JSON tunggal yang diminifikasi dengan satu kunci: "outline". Nilai dari "outline" harus berupa array berisi ${estimatedScenes} string ringkasan adegan.`;
                
                return ai.models.generateContent({
                    model: 'gemini-2.5-flash', contents: directorOutlinePrompt,
                    config: { responseMimeType: 'application/json', responseSchema: { type: Type.OBJECT, properties: { outline: { type: Type.ARRAY, items: { type: Type.STRING } } } } }
                });
            }, "LTX Script Outline");
            logUsage('text');
            ...
            if (!outline || !Array.isArray(outline) || outline.length === 0) {
                throw new Error("AI gagal membuat kerangka alur cerita yang valid.");
            }
            addLog(`[CT-LTX] Kerangka alur cerita dibuat. Langkah 2/2: Menulis detail untuk ${outline.length} adegan...`, 'status');
    
            // STEP 2: Generate details for each scene
            const newScenes: LtxScene[] = [];
            for (let i = 0; i < outline.length; i++) {
                const sceneOutline = outline[i];
                addLog(`[CT-LTX] Menulis detail untuk Adegan ${i + 1}/${outline.length}...`, 'status');
    
                const [sceneDetailResponse]: [GenerateContentResponse, any] = await executeApiCallWithKeyRotation(async (ai) => {
                    const charactersPrompt = ltxProject.characters.length > 0
                        ? `\n\n**Karakter yang Telah Ditentukan:**\n${ltxProject.characters.map(c => `- ${c.name}: ${c.description}. Pakaian: ${c.clothing}.`).join('\n')}`
                        : '';
                    const sceneDetailPrompt = `Anda adalah seorang penulis naskah dan direktur artistik.
                    **Konteks Film Keseluruhan:**
                    - Judul: ${ltxProject.title}, Genre: ${ltxProject.genre}, Gaya Visual: ${ltxProject.visualStyle}
                    - Alur Cerita Lengkap:\n${outline.map((o: string, idx: number) => `${idx + 1}. ${o}`).join('\n')}
                    ${charactersPrompt}
                    
                    **TUGAS ANDA:** Fokus HANYA pada bagian alur cerita untuk Adegan ${i + 1}: "${sceneOutline}"
                    
                    Berdasarkan bagian itu, buatlah:
                    1. 'description': Deskripsi naratif sinematik dari adegan.
                    2. 'imagePrompt': Prompt gambar yang sangat detail.
                    3. 'videoPrompt': Prompt video yang dinamis dengan arahan kamera.
                    4. 'audioPrompt': Ide untuk musik atau efek suara.
                    
                    Pastikan SEMUA prompt visual secara ketat mengikuti Gaya Visual ("${ltxProject.visualStyle}") dan konsisten dengan karakter yang telah ditentukan.
                    Kembalikan sebagai objek JSON tunggal yang diminifikasi.`;
    
                    return ai.models.generateContent({
                        model: 'gemini-2.5-flash', contents: sceneDetailPrompt,
                        config: { responseMimeType: 'application/json', responseSchema: { type: Type.OBJECT, properties: { description: { type: Type.STRING }, imagePrompt: { type: Type.STRING }, videoPrompt: { type: Type.STRING }, audioPrompt: { type: Type.STRING } } } }
                    });
                }, `LTX Scene Details ${i+1}`);
                logUsage('text');
    
                const sceneData = JSON.parse(sceneDetailResponse.text.replace(/\\`\\`\\`json|\\`\\`\\`/g, '').trim());
                newScenes.push({
                    id: generateRandomString(8), sceneNumber: i + 1, status: 'pending', transition: 'crossfade',
                    description: sceneData.description || '', imagePrompt: sceneData.imagePrompt || '', videoPrompt: sceneData.videoPrompt || '', audioPrompt: sceneData.audioPrompt || ''
                });
    ...
    const handleGenerateVisualsForScene = useCallback(async (sceneId: string, type: 'image' | 'video'): Promise<string | undefined> => {
        ...
        try {
            ...
            if (type === 'image') {
                ...
                const [operation, { apiKey: videoApiKey }] = await executeApiCallWithKeyRotation(
                ...
                addLog(`[CT-LTX Adegan ${scene!.sceneNumber}] Membuat narasi audio...`, 'status');
                const [audioResult]: [Blob, any] = await executeApiCallWithKeyRotation(
                    (ai) => generateTextToSpeech(ai, scene!.description),
                    `LTX Audio Scene ${scene.sceneNumber}`
                );
                const audioBlob = audioResult;
                const audioUrl = URL.createObjectURL(audioBlob);
                const audioFileName = `${getTimestamp()}_ltx_scene_${scene.sceneNumber}_audio.wav`;
                logUsage('audio');

                updateProject(p => ({ ...p, scenes: p.scenes.map(s => s.id === sceneId ? { ...s, status: 'done', videoUrl, videoFileName, audioUrl, audioFileName } : s) }));
    ...
    const handleMergeClips = async () => {
        ...
        const scenesToMerge = ltxProject.scenes.filter(s => s.status === 'done' && s.videoUrl && s.audioUrl);
        if (scenesToMerge.length < 1) {
            addLog('[CT-LTX] Butuh setidaknya 1 klip video & audio yang sudah dibuat untuk digabungkan.', 'warning');
            return;
        }
    
        setIsProcessing(true);
        addLog(`[CT-LTX] Memulai penggabungan untuk ${scenesToMerge.length} klip...`, 'status');
        ...
        try {
            ...
            const targetWidth = Math.round((targetHeight * w / h) / 2) * 2;
            ...
            addLog(`[CT-LTX] Unduhan film dimulai.`, 'info');
    ...
    }, [playAllState, scenesWithVideo, handleStopPlayAll, addLog]);
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                         ...
                         <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-700">
                            <Select label="Rasio Aspek" id="ltxAspectRatio" value={ltxProject.aspectRatio} onChange={e => updateProject({ aspectRatio: e.target.value })} disabled={isVeo3Model}>
                         ...
                         <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <Select label="Model Video" id="ltxVideoModel" value={ltxProject.videoModel} onChange={e => updateProject({ videoModel: e.target.value })}>
