...
const BatchTxtGenerator: React.FC = () => {
  const { t } = useLanguage();
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
        ...
        const loadedPrompts = text.split('\n').map(p => p.trim()).filter(Boolean);
  ...
  const handleDownload = (url: string, jobIndex: number) => {
    ...
    link.href = url;
    link.download = `ct-batch-txt-job-${jobIndex + 1}.mp4`;
        ...
        <form onSubmit={handleSubmit} className="space-y-6 flex flex-col">
            <fieldset disabled={isLoading} className="space-y-6 flex-grow">
              ...
              <div className="space-y-2 text-center p-4 bg-slate-800/50 rounded-lg border border-slate-700">
                <input type="file" ref={fileInputRef} onChange={handleFileChange} accept=".txt" hidden />
                <Button type="button" onClick={() => fileInputRef.current?.click()} className="w-full justify-center">{t('selectTxtFile')}</Button>
                {prompts.length > 0 && <p className="text-sm text-green-500 mt-2">{t('promptsLoaded').replace('{count}', String(prompts.length))}</p>}
              </div>
...
export default BatchTxtGenerator;
